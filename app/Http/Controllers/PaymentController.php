    ...
    public function pay($id): View
{
   ...
   if (empty($transaction->midtrans_order_id)) {
        ...
        // Jika belum ada payment_url atau masih pending
        ...
        // SOLUSI UTAMA: Convert QR ke Base64 langsung di sini
        if ($qrUrl) {
            $qrCodeBase64 = $this->getQrAsBase64($qrUrl);
        }

        $isPaid = strtolower($transaction->payment_status) === 'paid';

        return view('payments.pay', compact('transaction', 'qrUrl', 'deeplinkUrl', 'isPaid', 'errorMessage', 'qrCodeBase64'));
    }

    /**
     * SATU-SATUNYA function untuk handle QR Code
     */
    private function getQrAsBase64($qrUrl)
    {
        try {
            // Pastikan HTTPS
            $qrUrl = str_replace('http://', 'https://', $qrUrl);
            
            // Setting untuk Railway
            ...
            \Log::error('QR conversion error: ' . $e->getMessage());
        }
