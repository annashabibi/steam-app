...
class CategoryController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        if ($request->search) {
            ...
            $categories = Category::select('id', 'type_motor')
                ->where('type_motor', 'LIKE', '%' . $request->search . '%')
                ->paginate($pagination)
                ->withQueryString();
        } else {
    ...
    public function store(Request $request): RedirectResponse
    {
        // validasi form
        $request->validate([
            'type_motor' => 'required|unique:categories'
        ]);
    ...
    public function update(Request $request, $id): RedirectResponse
    {
        // validasi form
        $request->validate([
            'type_motor' => 'required|unique:categories,type_motor,' . $id
        ]);
